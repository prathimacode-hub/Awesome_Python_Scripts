...
class Vehicles:
    ...
    def equip_unequip_button(self, btn_index, m_x, m_y, left_click):
        ...
        if BUTTON.x + BUTTON.width > m_x > BUTTON.x and BUTTON.y + BUTTON.height > m_y > BUTTON.y:
            ...
            if btn_index == 2 and left_click == True:
                BUTTON_CLICK_SOUND.play()
                self.equip('MainDb.db', 'cartable', 'is_equiped', self.row_num)
            elif btn_index == 3 and left_click == True:
                BUTTON_CLICK_SOUND.play()
                self.unequip('MainDb.db', 'cartable', 'is_equiped', self.row_num)
    ...
    def buy(self,total_coins):
        ...
        if total_coins >= self.cost and self.bought !=1:
            APPLAUSE_SOUND.play()
            self.update_database_for_buy('MainDB.db', 'cointable', 'cartable', self.row_num, 'coin_collected', 'is_purchased', total_coins-self.cost, 1)
        else:
            MESSAGE_SOUND.play()
...
def room_one_loop():
    ...
    while run:
        ...
        lst = fetch_all_data('MainDB.db', 'cartable')
        coin_lst = fetch_all_data('MainDB.db', 'cointable')
