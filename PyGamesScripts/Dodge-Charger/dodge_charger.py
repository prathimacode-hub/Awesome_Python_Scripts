...
import random
import time
from garage_menu import fetch_all_data
from garage_menu import COINS
from garage_menu import draw_coin
from garage_menu import room_one_loop
...
def game_loop():
    ...
    run = True
    bg_posy = 0
    num, name, cost, speed, health, bought, equiped = get_player_car_data('MainDB.db', 'cartable')
...
def end_loop():
    ...
    # update coin in db
    coins_lst = fetch_all_data("MainDB.db", "cointable")
    total_coins = coins_lst[0][0]
    total_coins += coins_collected
    update_db('MainDB.db', 'cointable', 'coin_collected', total_coins)
    while run:
        clock.tick(FPS)
