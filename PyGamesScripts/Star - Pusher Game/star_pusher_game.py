...
def main():
    ...
    # Because the Surface object stored in DISPLAYSURF was returned from the pygame.
    ...
    PLAYERIMAGES = [IMAGESDICT['princess'], # PLAYERIMAGES is a list of all possible characters the player can be.
                    IMAGESDICT['boy'],
                    IMAGESDICT['catgirl'],
                    IMAGESDICT['horngirl'],
                    IMAGESDICT['pinkgirl']]
...
def makeMove(mapObj, gameStateObj, playerMoveTo):
    ...
    # This variable is "syntactic sugar". Typing "stars" is more readable than typing "gameStateObj['stars']" in our code.
    stars = gameStateObj['stars']

    # The code for handling each of the directions is so similar aside from adding or subtracting 1 to the x/y coordinates. We can
...
def startScreen():
    ...
    # Unfortunately, Pygame's font & text system only shows one line at a time, so we can't use strings with \n newline characters in them.
...
def readLevelsFile(filename):
    ...
    for lineNum in range(len(content)):
        ...
        elif line == '' and len(mapTextLines) > 0:  # A blank line indicates the end of a level's map in the file.
...
def drawMap(mapObj, gameStateObj, goals):
    ...
    for x in range(len(mapObj)):
        ...
        for y in range(len(mapObj[x])):
            ...
            mapSurf.blit(baseTile, spaceRect) # First draw the base ground/wall tile.

            if mapObj[x][y] in OUTSIDEDECOMAPPING:  # Draw any tree/rock decorations that are on this tile.
               
                mapSurf.blit(OUTSIDEDECOMAPPING[mapObj[x][y]], spaceRect)
                
            elif (x, y) in gameStateObj['stars']:
                if (x, y) in goals:  # A goal AND star are on this space, draw goal first.
            ...
            elif (x, y) in goals:  # Draw a goal without a star on it.
